...
.dc{max-width:400px;position:relative}
...
<div class="container">
  ...
  <div class="tw"><table><thead><tr>
    ...
    <th>Категория</th><th>Ключевые слова</th><th>Теги</th>
    <th>ОКПД2</th><th>Макс. цена</th><th>Дедлайн</th>
    <th class="sortable" onclick="ss('discovered_at')">Найден<span class="arrow" id="arr-discovered_at"></span></th>
  </tr></thead><tbody id="tb">
    <tr><td colspan="12" class="ld">Загрузка...</td></tr>
  </tbody></table></div>
...
def _js() -> str:
    ...
    rub = _RUB
    return f"""<script>
const A='/api/v1';let cp=1;const pp=30;
...
b.innerHTML='<tr><td colspan="12" class="ld">\\u0417\\u0430\\u0433\\u0440\\u0443\\u0437\\u043A\\u0430...</td></tr>';
...
}}catch(er){{b.innerHTML='<tr><td colspan="12" class="em">\\u041E\\u0448\\u0438\\u0431\\u043A\\u0430 \\u0437\\u0430\\u0433\\u0440\\u0443\\u0437\\u043A\\u0438</td></tr>';console.error(er)}}}}
function rl(d){{const b=document.getElementById('tb');
if(!d.items||!d.items.length){{b.innerHTML='<tr><td colspan="12" class="em">\\u041D\\u0435\\u0442 \\u043B\\u0438\\u0434\\u043E\\u0432</td></tr>';return}}
...
(long?'<span class="dc-toggle" onclick="td('+i+')">\\u0420\\u0430\\u0437\\u0432\\u0435\\u0440\\u043D\\u0443\\u0442\\u044C</span>':'')+
'</div></td>'+
'<td>'+(l.budget?'<span class="bud">'+Number(l.budget).toLocaleString("ru-RU")+' {rub}</span>':'\\u2014')+'</td>'+
'<td>'+(l.category?'<span class="b b-cat">'+e(l.category)+'</span>':'\\u2014')+'</td>'+
'<td>'+((l.matched_keywords||[]).map(k=>'<span class="b b-kw">'+e(k)+'</span>').join(' ')||'\\u2014')+'</td>'+
'<td>'+((l.tags||[]).map(t=>'<span class="b b-tag">'+e(t)+'</span>').join(' ')||'\\u2014')+'</td>'+
'<td>'+((l.okpd2_codes||[]).map(c=>'<span class="b b-ok">'+e(c)+'</span>').join(' ')||'\\u2014')+'</td>'+
'<td>'+(l.max_contract_price?'<span class="bud">'+Number(l.max_contract_price).toLocaleString("ru-RU")+' {rub}</span>':'\\u2014')+'</td>'+
'<td class="dt">'+(l.submission_deadline?fd(l.submission_deadline):'\\u2014')+'</td>'+
'<td class="dt">'+fd(l.discovered_at)+'</td>'+
'</tr>'}}).join('')}}
function td(i){{const el=document.getElementById('desc-'+i);if(!el)return;
